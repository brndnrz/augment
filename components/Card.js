...
const Card = ({ article }) => {
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer">
      <div className="w-full max-w-sm mx-auto mt-8 cursor-pointer h-80 sm:h-96 sm:w-10/12 lg:h-[34rem] rounded-2xl relative">
        <img
        ...
        <div className="absolute bottom-0 left-0 flex items-center w-full p-2 text-center rounded-2xl h-2/6 backdrop-blur-sm">
          <h1 className="w-4/5 mx-auto text-sm text-xl line-clamp-3 text-backgroundWhite sm:text-base md:text-lg font-Inter">
            {title}
          </h1>
        </div>
      </div>
